...
test("square room includes 0,0 and includes outer border", () => {
  const sq = { kind: "square" as const, width: 5, height: 5 };
  expect(contains(sq, { x: 0, y: 0 })).toBe(true);
  expect(contains(sq, { x: 4, y: 4 })).toBe(true);
  expect(contains(sq, { x: 5, y: 0 })).toBe(true);
  expect(contains(sq, { x: 0, y: 5 })).toBe(true);
  expect(contains(sq, { x: 6, y: 0 })).toBe(false);
  expect(contains(sq, { x: 0, y: 6 })).toBe(false);
});
